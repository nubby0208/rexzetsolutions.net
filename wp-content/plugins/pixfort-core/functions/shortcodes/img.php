	...
	function sc_pix_img( $attr, $content = null ){
        ...
        if(!empty($image)){
            ...
            if($link){
                ...
                $output .= '<a href="'.$link.'" '.$ntab.' class="pix-img-el '.$class_names.' '.$rounded_img.'" '.$jarallax.'>';
				...
				if(!empty($imgSrcset)){
					$output .= '<img class="pix-lazy2 card-img2 '.$rounded_img.' h-1002" src="'.$imgSrc.'" srcset="'.$imgSrcset.'" '.$imgWidth.' '.$imgHeight.' alt="'. $alt .'" '.$inline_style.'/>';
				}else{
					$output .= '<img class="pix-lazy2 card-img2 '.$rounded_img.' h-1002" src="'.$imgSrc.'" '.$imgWidth.' '.$imgHeight.' alt="'. $alt .'" '.$inline_style.'/>';
				}
